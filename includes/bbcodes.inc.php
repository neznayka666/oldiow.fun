...
$bbcodes = array("/&/is" => "&amp;","/\"/is" => "&quot;","/\'/is" => "\'","/</is" => "&lt;","/>/is" => "&gt;","/\r\n/is" => "<br />","/\n/is" => "<br />","/\[b\](.*?)\[\/b\]/is" => "<b>$1</b>","/\[i\](.*?)\[\/i\]/is" => "<i>$1</i>","/\[u\](.*?)\[\/u\]/is" => "<u>$1</u>","/\[url\](.*?)\[\/url\]/is" => "<a href=$1>$1</a>");
$decodes = array("/&amp;/is" => "&","/&quot;/is" => "\"","/\'/is" => "\'","/&lt;/is" => "<","/&gt;/is" => ">","/\<br \/\>/is" => "\r\n","/\<b\>(.*?)\<\/b\>/is" => "[b]$1[/b]","/\<i\>(.*?)\<\/i\>/is" => "[i]$1[/i]","/\<u\>(.*?)\<\/u\>/is" => "[u]$1[/u]","/\<a href\=(.*?)\>(.*?)\<\/a\>/is" => "[url]$1[/url]");
function bbCodes($text){
	global $pers,$smiles,$bbcodes;
	$text=preg_replace("/(\\\\[0-9]*)/i",'',$text);
	$text = preg_replace(array_keys($bbcodes), array_values($bbcodes), $text);
	for($k=0; $k<count($smiles); $k++){
		$forum_smiles = explode("|",$pers['forum_smiles']);
		if(in_array(pow(2,$k),$forum_smiles)){
			for($m=0; $m<count($smiles[$k]); $m++){
				$text = preg_replace("/:".$smiles[$k][$m][0].":/",'<img src=http://'.IMG.'/forum/smiles/'.$smiles[$k][$m][0].'.gif width='.$smiles[$k][$m][1].' height='.$smiles[$k][$m][2].' border=0 align=absmiddle>',$text);
			}
		}
	}
	return $text;
...
function deCodes($text){
	...
	for($k=0; $k<count($smiles); $k++){
		...
		if(in_array(pow(2,$k),$forum_smiles)){
			for($m=0; $m<count($smiles[$k]); $m++){
				$text = preg_replace('/\<img src=http:\/\/'.IMG.'\/forum\/smiles\/'.$smiles[$k][$m][0].'.gif width='.$smiles[$k][$m][1].' height='.$smiles[$k][$m][2].' border=0 align=absmiddle\>/',":".$smiles[$k][$m][0].":",$text);
			}
		}
	}
	return $text;
