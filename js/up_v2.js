...
//	d.write ('<body topmargin="0" style="word-spacing: 0; margin-left: 0; margin-right: 0" leftmargin=0 onresize="on_resize()">');
	if (curstate!=4)
	{
		...
		if (fourthname != '' && !trvm && !apps)
			fourth = "<img src='/images/nav/top/world.gif' " + code + " id='but7' " + mouser(fourthname) + " style='cursor: pointer;'>";
		...
		d.write('<img src="/images/nav/top/inv.gif" onclick="location=\'main.php?go=inv\'" '+inv+' id="but2" '+mouser("Инвентарь")+' style="cursor:pointer;">'); 
		d.write('' + fourth + ''); 		
		...
		// Вывод инфы
		d.write("<div id=_top style='width:100%; margin: 0 0 10px 0;padding-top: 40px;'></div>");
	}
	if(help==2)
	{
...
function sbox(t)
{
	return '<div align=left><div class="corners"><div class="inner"><div class="content">'+t+' 111</div></div></div></div>';
}


function sbox2(t,c)//поединки
{
...
function sbox2b(c)
{
	...
	return '<table cellspacing="0" cellpadding="0" style="position:relative;width: 100%;"> <tr>  <td style="'+c+'">';
}
