<?php
	...
	{
		$p = $db->sqla("SELECT uid,user FROM users WHERE user='".$_POST["friend_nick"]."'");
		if ($p)
		{
	...
	{
		$db->sql("UPDATE friendship SET type=0 WHERE uid1=".intval($_GET["get_friendship"]));
	}
		
	if (@$_GET["cans_friendship"])
	{
		$db->sql("DELETE FROM friendship WHERE uid1=".intval($_GET["cans_friendship"])." and uid2=".intval($player->pers["uid"]));
		$db->sql("DELETE FROM friendship WHERE uid2=".intval($_GET["cans_friendship"])." and uid1=".intval($player->pers["uid"]));
	}
?>
<SCRIPT src="js/self.js"></SCRIPT>
<table style="width: 1200px;margin:0 auto;" class="greyBlock">
    <tr>
        <td><b>Список друзей:</b></td>
    </tr>
    <tr>
        <td align=center>
            <table border="0" width="1200px" cellspacing="5" cellpadding="5" class=greyBlocke>
                <?php
	$q = '';
	$q2 = '';
	...
	{
	echo"<tr><td>";
	echo"<img src='/images/nav/chat/private.png' onclick=\"javascript:top.say_private('".$perssost["user"]."')\" style=cursor:pointer> &nbsp;&nbsp;";
	echo "<img src='images/signs/".$perssost["sign"].".gif' title='".$perssost["state"]."'>";
	echo " <b>".$perssost["user"]."</b> [<font class=lvl>".$perssost["level"]."</font>]";
	echo "<img src='images/icons/inf.gif' onclick=\"javascript:window.open('info.php?p=".$perssost["user"]."','_blank')\" style=cursor:pointer>";
	echo "</td>";
...
?>
    ...
    <tr>
        <td>Заявки на дружбу с вами:</td>
    </tr>
    <tr>
        <td align=center>
            <table border="0" width="80%" cellspacing="0" cellpadding="0" class=greyBlocke>
                <?
	if ($q2)
	{
